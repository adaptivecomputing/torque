...
ok(defined $pid, 'TORQUE Currently Running') or
  BAIL_OUT('pbs_server not running, start pbs_server and rerun test - see Section XXX');

# qmgr - TORQUE Database Created
my $qmgr = `qmgr -c 'p s'` || undef;
ok(defined $qmgr, 'Database Created') or
  BAIL_OUT("TORQUE database not created, restart pbs_server using 'pbs_server -t create' - see Section XXX");
