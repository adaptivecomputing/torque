...
dnl you want, hmm, well, atleast it has --cflags, --version, --libs.
...
dnl you don't even need to distribute one along. Place this macro in
dnl your configure.ac, et voila, you got one that you want to install.
dnl
dnl oh, btw, if the first arg looks like "mylib -lwhat' then it will go
dnl to be added to the --libs, and mylib is extracted.
dnl
dnl the defaults: $1 = $PACKAGE $LIBS $2 = $VERSION there is also an
dnl AC_SUBST(GENERIC_CONFIG) that will be set to the name of the file
dnl that we did output in this macro. Use as:
dnl
...
echo '  --includedir)            o="$o $includedir" ;;' >>$F
