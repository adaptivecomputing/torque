...
START_TEST(test_get_job_sid_from_pid)
  ...
  job_sid_set.insert(1000);

  /* create space for 2 pids */
  proc_array = (proc_stat_t *) calloc(sizeof(proc_stat_t), 2);
  ...
  /* pid's sid is in job_sid_set so expect success */
  fail_unless(get_job_sid_from_pid(10) == 1000);

  /* change the pid's session to something not in job_sid_set */
  ...
  /* make pid 20's session id be something not in the job_sid_set */
  proc_array[index].session = 2000;

  /* pid 20's session is not in job_sid_set and has parent pid 1 so expect failure */ 
...
START_TEST(test_injob)
  ...
  /* set job to have 5000 as its session id */
  pjob->ji_job_pid = 5000;

  /* expect failure since pjob->ji_job_pid and job session id of pid don't match */
  fail_unless(injob(pjob, 10) == FALSE);
  }
END_TEST
...
START_TEST(test_cput_sum)
  ...
  proc_array = (proc_stat_t *) calloc(sizeof(proc_stat_t), 1);
  fail_unless(proc_array != NULL);
