...
int socket_connect_addr(int *local_socket, struct sockaddr *remote, size_t remote_size, int is_privileged, char **err_msg,bool checkForReads);
int socket_wait_for_write(int socket);
int socket_wait_for_read_or_write(int  socket,bool &readyToWrite,bool &errorIsWriteError);
...
int wait_request(time_t waittime, long *SState,int writeFd);
