...
#define  DCGM_array_sizes 10 // At the time this code was ritten this was the hard coded array sizes
                             // for xidCriticalErrorsTs, computePidInfo and graphicsPidInfo
	...
	for (unsigned int i = 0; i < DCGM_array_sizes; i++)
	  {
		...
		memset(&this->computePidInfo[i], 0, sizeof(dcgmProcessUtilInfo_t));
		memset(&this->graphicsPidInfo[i], 0, sizeof(dcgmProcessUtilInfo_t));
		}
	}
...
//	for (unsigned int i = 0; i < DCGM_array_sizes; i++)
//					this->computePidInfo[i] = other.computePidInfo[i];
...
//
//	for (unsigned int i = 0; i < DCGM_array_sizes; i++)
//					this->computePidInfo[i] = gpu_usage_info.computePidInfo[i];
//
...
//        snprintf(buf, sizeof(buf), "{\"%d\": %ld}", i, this->computePidInfo[i]);
//			else
 //       snprintf(buf, sizeof(buf), "{\"%d\": %ld},", i, this->computePidInfo[i]);
  //    usage_info += buf;
	//		}
...
//    {
//    snprintf(buf, sizeof(buf), "\"graphicsPidInfo\": [");
//    usage_info += buf;
//    for (unsigned int i = 0; i < this->numGraphicsPids; i++)
//		  
			/* Don't put a comma after the last entry in the array */
//		if (i == this->numGraphicsPids - 1)
 //       snprintf(buf, sizeof(buf), "{\"%d\": %ld}", i, this->graphicsPidInfo[i]);
//			else
//        snprintf(buf, sizeof(buf), "{\"%d\": %ld},", i, this->graphicsPidInfo[i]);
//      usage_info += buf;
//			}
