...
START_TEST(test_pop_job_from_recycler)
  ...
  //simulate having job structure pointing to a deleted memory
    strcpy(pjobs[2]->ji_qs.ji_jobid, "hello"); 
    free(pjobs[2]);
    ...
    fail_unless(strcmp(pjob->ji_qs.ji_jobid, buf)==0);
    pjob = pop_job_from_recycler(&recycler.rc_jobs);
    }

    fail_unless(count == 4, "count of valid recycled jobs were wrong");
  }
END_TEST
