...
char *server_host = NULL; /* must use a heap storage, else it crashes */
...
int set_pbs_server_name()
  ...
  server_host = (char *)calloc(PBS_MAXHOSTNAME + 1, sizeof(char));
  for(p = info; p != NULL; p = p->ai_next)
    {
    snprintf(server_host, PBS_MAXHOSTNAME + 1, "%s", p->ai_canonname);
    break;
    }
