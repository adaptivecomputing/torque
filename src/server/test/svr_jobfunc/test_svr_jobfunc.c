...
START_TEST(svr_enquejob_test)
  ...
  result = svr_enquejob(NULL, 0, 0);
  fail_unless(result != PBSE_NONE, "NULL input pointer fail");

  result = svr_enquejob(&test_job, 0, 0);
...
START_TEST(svr_dequejob_test)
  ...
  result = svr_dequejob("", 0);
  fail_unless(result == PBSE_JOBNOTFOUND, "empty input string fail");

  result = svr_dequejob(job_id, 0);
