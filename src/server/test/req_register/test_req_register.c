...
struct depend_job *make_dependjob(struct depend *pdep, char *jobid, char *host);
...
void fast_strcat(char **Dest, char *Src);
int dup_depend(pbs_attribute *pattr, struct depend *pd);
void cat_jobsvr(char **Dest, char *Src);
int decode_depend(pbs_attribute *pattr, char *name, char *rescn, char *val, int perm);
int encode_depend(pbs_attribute *pattr, tlist_head *phead, char *atname, char *rsname, int mode, int perm);
