...
#include "attribute.h" /* attribute, attribute_def, svrattrl, batch_op */
#include "batch_request.h" /* batch_request */
#include "work_task.h" /* work_task */

/* static char *check_que_attr(pbs_queue *pque); */

int check_que_enable(attribute *pattr, void *pque, int mode);
...
/* static int mgr_set_attr(attribute *pattr, attribute_def *pdef, int limit, svrattrl *plist, int privil, int *bad, void *parent, int mode); */

int mgr_unset_attr(attribute *pattr, attribute_def *pdef, int limit, svrattrl *plist, int privil, int *bad);
...
int servername_chk(attribute *pattr, void *pobject, int actmode);

void on_extra_resc(struct work_task *ptask);

int extra_resc_chk(attribute *pattr, void *pobject, int actmode);

void free_extraresc(struct attribute *attr);

int disallowed_types_chk(attribute *pattr, void *pobject, int actmode);

/* static int mgr_long_action_helper(attribute *pattr, int actmode, long minimum, long maximum); */

int schiter_chk(attribute *pattr, void *pobject, int actmode);

int nextjobnum_chk(attribute *pattr, void *pobject, int actmode);

int set_nextjobnum(struct attribute *attr, struct attribute *new, enum batch_op op);
