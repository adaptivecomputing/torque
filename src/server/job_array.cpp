...
int create_and_queue_array_subjob(job_array *pa, std::shared_ptr<mutex_mgr> &array_mgr, job *template_job,
                                  std::shared_ptr<mutex_mgr> &template_job_mgr, int index, std::string &prev_job_id,
                                  bool place_hold);
...
void job_array::create_job_if_needed()
  ...
  if (next_index >= 0)
    ...
    if (template_job != NULL)
      {
      std::shared_ptr<mutex_mgr> template_mgr = create_managed_mutex (template_job->ji_mutex, true, rc);
	  if (rc != PBSE_NONE)
		{
// @todo should throw
		return;
      ...
      std::shared_ptr<mutex_mgr> array_mgr = create_managed_mutex(this->ai_mutex, true, rc);
	  if (rc != PBSE_NONE)
		{
// @todo should throw
		return;
        ...
        if (this->need_to_update_slot_limits() == true)
          ...
          for (i = 0; i < this->ai_qs.array_size; i++)
            ...
            if ((pj = svr_find_job(this->job_ids[i], TRUE)) == NULL)
              ...
              {
			  int rc;
              std::shared_ptr<mutex_mgr> pj_mutex = create_managed_mutex(pj->ji_mutex, true, rc);
			  if ( rc != PBSE_NONE)
				{
				// @todo should throw
				return;
