...
START_TEST(test_depend_on_que)
  ...
  pdep = make_depend(JOB_DEPEND_TYPE_AFTERNOTOK, pjob->get_attr(JOB_ATR_depend));
  make_dependjob(pdep, job2);

  // These three modes should do nothing
  fail_unless(depend_on_que(pjob->get_attr(JOB_ATR_depend), pjob, ATR_ACTION_NEW) == PBSE_NONE);
  fail_unless(depend_on_que(pjob->get_attr(JOB_ATR_depend), pjob, ATR_ACTION_FREE) == PBSE_NONE);
  fail_unless(depend_on_que(pjob->get_attr(JOB_ATR_depend), pjob, ATR_ACTION_RECOV) == PBSE_NONE);

  fail_unless(depend_on_que(pjob->get_attr(JOB_ATR_depend), pjob, ATR_ACTION_ALTER) == PBSE_NONE);
