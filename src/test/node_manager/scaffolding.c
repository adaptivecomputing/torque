...
svr_job *svr_find_job(const char *jobid, int get_subjob)
  ...
  if (strstr(jobid, "lei.ac"))
    ...
    return(NULL);
           ...
           (!strcmp(jobid, "5")))
    {
    pjob.set_str_attr(JOB_ATR_exec_host, strdup("tom/0"));
    }
    ...
    {
    pjob.set_str_attr(JOB_ATR_exec_host, strdup("bob/5"));
    }
  else
...
void svr_job::set_plugin_resource_usage_from_json(Json::Value &usage) {}

bool have_incompatible_dash_l_resource(svr_job *pjob)
  {
  return(true);
  }
