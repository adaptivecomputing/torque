...
START_TEST(test_job_constructor)
  ...
  pjob.set_str_attr(JOB_ATR_jobname, strdup(jname));
  pjob.set_str_attr(JOB_ATR_job_owner, strdup(owner));
  pjob.set_str_attr(JOB_ATR_exec_host, strdup(hlist));
  pjob.set_str_attr(JOB_ATR_join, strdup(join));
  pjob.set_str_attr(JOB_ATR_outpath, strdup(outpath));
  pjob.set_str_attr(JOB_ATR_init_work_dir, strdup(outpath));
  pjob.set_resc_attr(JOB_ATR_resource, &resources);
  pjob.set_resc_attr(JOB_ATR_resc_used, &resources);

  pjob.set_queue(queue);
  pjob.set_jobid(jid);
