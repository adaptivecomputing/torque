...
bool is_user_allowed_to_submit_jobs(svr_job *pjob, const char *luser, char *EMsg, int logging);
...
START_TEST(test_is_user_allowed_to_submit_jobs)
  ...
  pjob.set_str_attr(JOB_ATR_job_owner, strdup("dbeer@roshar.cosmere"));
  var = NULL;
  exists = false;
