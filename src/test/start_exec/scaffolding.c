...
int move_to_job_cpuset(pid_t, mom_job *) { return 0; }
int diswsi(tcp_chan *chan, int i) { return 0; }
int encode_DIS_svrattrl(tcp_chan *chan, svrattrl *s) { return 0; }
int im_compose(tcp_chan *chan, const char *arg2, const char *a3, int a4, int a5, unsigned int a6) { return 0; }
int create_alps_reservation(const char *a1, const char *a2, const char *a3, char *a4, char *a5, long long a6, int a7, int a8, int a9, char **a10,const char *a11, std::string& cray_frequency) { return 0; }
...
int x11_create_display(int x11_use_localhost, char *display, char *phost, int pport, char *homedir, const char *x11authstr)
  {
...
int set_job(mom_job *pjob, struct startjob_rtn *sjr)
  {
...
int send_sisters(mom_job *pjob, int com, int using_radix, std::set<int> *sisters_to_contact)
  {
  send_sisters_called++;
...
int site_mom_chkuser(mom_job *pjob)
  {
...
int site_job_setup(mom_job *pjob)
  {
  fprintf(stderr, "The call to site_job_setup needs to be mocked!!\n");
  exit(1);
  }

int set_mach_vars(mom_job *pjob, struct var_table *vtab)
  {
...
char * set_shell(mom_job *pjob, struct passwd *pwdp)
  {
...
void get_chkpt_dir_to_use(mom_job *pjob, char *chkpt_dir, int size)
  {
...
long long get_memory_requested_in_kb(mom_job&)
  {
  return 0;
  }

int get_cpu_count_requested_on_this_node(mom_job&)
  {
  return 0;
  }
...
int trq_cg_create_cpuset_cgroup(mom_job*, int)
  {
  return(PBSE_NONE);
  }
...
mom_job::mom_job() : ji_tasks(), ji_usages()
  {
  }
mom_job::~mom_job() {}
...
int job::set_ll_attr(int index, long long ll)
  {
