...
extern int set_nodes_attr(svr_job *pjob);
...
int fill_resource_list(svr_job *pj, xmlNodePtr resource_list_node, char *log_buf, size_t buflen, const char *aname);
int saveJobToXML(svr_job *pjob, const char *filename);
void init_resc_defs();
...
int  set_array_job_ids(svr_job **pjob, char *log_buf, size_t buflen);
svrattrl *fill_svrattr_info(const char *aname, const char *avalue, const char *rname, char *log_buf, size_t      buf_len);
void decode_attribute(svrattrl *pal, svr_job *pjob, bool freeExisting);
job_array *ghost_create_jobs_array(svr_job *pjob, const char *array_id);
void check_and_reallocate_job_ids(job_array *pa, int index);
void update_recovered_array_values(job_array *pa, svr_job *pjob);
...
START_TEST(ghost_array_test)
  ...
  pjob->set_jobid("10[0].napali");

  init();

  pjob->set_str_attr(JOB_ATR_job_owner, strdup("dbeer@napali"));
  ...
  fail_unless(!strcmp(pa->job_ids[0], pjob->get_jobid()));
...
START_TEST(test_decode_attribute)
  ...
  fail_unless(pjob->get_long_attr(JOB_ATR_hold) == 1, "val: %d", (int)pjob->get_long_attr(JOB_ATR_hold));
...
START_TEST(ghost_array_test2)
  ...
  pjob->set_str_attr(JOB_ATR_job_owner, strdup("dbeer@napali"));
  pjob->set_long_attr(JOB_ATR_job_array_id, 500); // >DEFAULT_ARRAY_RECOV_SIZE
...
void fill_other_job_info(svr_job *pjob, int resc_access_perm)
  {
  char buf[256];

  snprintf(buf, sizeof(buf), "%s", "echan@lei");

  pjob->set_str_attr(JOB_ATR_job_owner, buf);

  time_t time_now = time(NULL);

  pjob->set_long_attr(JOB_ATR_ctime, time_now);

  /* set hop count = 1 */
  pjob->set_long_attr(JOB_ATR_hopcount, 1);

  pjob->set_str_attr(JOB_ATR_outpath, strdup("/someOutput/STDIN.o945"));

  pjob->set_str_attr(JOB_ATR_errpath, strdup("/someOutput/STDIN.e945"));
  pjob->set_str_attr(JOB_ATR_at_server, server_name);
   
  pjob->set_queue("batch");
  pjob->set_long_attr(JOB_ATR_substate, JOB_SUBSTATE_TRANSIN);

  /* set remaining job structure elements */
  pjob->set_state(JOB_STATE_TRANSIT);
  pjob->set_substate(JOB_SUBSTATE_TRANSIN);
  pjob->set_long_attr(JOB_ATR_mtime, (long)time_now);
  pjob->set_un_type(JOB_UNION_TYPE_NEW);
  pjob->set_fromsock(9);
...
svr_job *create_a_job(const char *jobId)
  {
  ...
  pj->set_jobid(jobId);
  pj->set_fileprefix(jobId);
  pj->set_modified(true);
  pj->set_svrflags(JOB_SVFLG_HERE);
...
int job_compare_fix_fields(svr_job *pjob1, svr_job* pjob2)
  {
  if (pjob1->get_qs_version() != pjob2->get_qs_version())
    return -1;
  if (pjob1->get_state() != pjob2->get_state())
    return -1;
  if (pjob1->get_substate() != pjob2->get_substate()) 
    return -1;
  if (pjob1->get_svrflags() != pjob2->get_svrflags())
    return -1;
  if (pjob1->get_start_time() != pjob2->get_start_time())
    return -1;
  if (strcmp(pjob1->get_jobid(), pjob2->get_jobid()))
    return -1;
  if (strcmp(pjob1->get_fileprefix(), pjob2->get_fileprefix()))
    return -1;
  if (strcmp(pjob1->get_queue(), pjob2->get_queue()))
    return -1;
  if (strcmp(pjob1->get_destination(), pjob2->get_destination()))
    return -1;

  return 0;
  }

int job_compare_union_fields(svr_job *pjob1, svr_job *pjob2)
  {
  if (pjob1->get_fromaddr() != pjob2->get_fromaddr())
    return -1;
  if (pjob1->get_fromsock() != pjob2->get_fromsock())
    return -1;
  if (pjob1->get_scriptsz() != pjob2->get_scriptsz())
    return -1;

  return 0;
  }

int job_compare_attributes(svr_job *pjo1, svr_job *pj2)
  {
  return 0;
  }

int job_compare(svr_job *pjob1, svr_job *pjob2)
  {
...
START_TEST(test_job_recover)
  ...
  svr_job *recov_pj = job_recov(jobFileName);
