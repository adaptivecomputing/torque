	...
	switch( pbs_state )
	 {
		...
		case 'E': /* Job is exiting after having run. */
			...
			break;
		case 'H': /* Job is held. */
			*remote_ps = DRMAA_PS_USER_ON_HOLD;  break;
		case 'Q': /* Job is queued, eligible to run or routed. */
			*remote_ps = DRMAA_PS_QUEUED_ACTIVE;  break;
		...
		case 'T': /* Job is being moved to new location. */
			*remote_ps = DRMAA_PS_RUNNING;  break;  /* ?? */
		case 'W': /* Job is waiting for its execution time to be reached. */
			*remote_ps = DRMAA_PS_USER_ON_HOLD;  break;
		case 'S': /* (Unicos only) job is suspend. */
			*remote_ps = DRMAA_PS_SYSTEM_SUSPENDED;  break;  /* ?? */
		case 0:  default:
			*remote_ps = DRMAA_PS_UNDETERMINED;  break;
	 }
	pbs_statfree( status );
