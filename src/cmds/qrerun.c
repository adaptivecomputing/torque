...
int main(
  ...
  char rmt_server[MAXSERVERNAME+1];
  ...
  for (;optind < argc;optind++)
    ...
    if (get_server_and_job_ids(job_id, id_list, server_name))
      ...
      continue;
		...
		server_out = server_name;

cnt:

    connect = cnt2server(server_out.c_str(), false);

    if (connect <= 0)
      {
      any_failed = -1 * connect;

      if (server_out.size() != 0)
				{
				std::cout << "qrerun: cannot connect to server " << server_out << " (errno=" << any_failed << ") " << pbs_strerror(any_failed) << std::endl;
				}
      else
				{
				std::cout << "qrerun: cannot connect to server " << pbs_server << " (errno=" << any_failed << ") " << pbs_strerror(any_failed) << std::endl; 
				}
      continue;
      }
    ...
    if (stat && (any_failed != PBSE_UNKJOBID))
      ...
      if (locate_job(job_id_out, server_out.c_str(), rmt_server))
        {
        pbs_disconnect(connect);

				server_out = rmt_server;
