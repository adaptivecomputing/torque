...
int hash_add_hash(job_data **dest, job_data *src, int overwrite_existing)
  {
  printf("Error, stub must be completed");
  exit(1);
  }
...
int hash_del_item(job_data **head, char *name)
  {
  printf("Error, stub must be completed");
  exit(1);
  }
...
void hash_add_or_exit(job_data **head, char *name, char *value, int var_type)
  {
  printf("Error, stub must be completed");
  exit(1);
  }
