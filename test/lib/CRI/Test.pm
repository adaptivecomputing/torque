...
# 
# File   :  CRI/Test.pm
# History:  14-sep-2009 (jdayley) Added this headers and updated 
#           comments.
#
########################################################################
#
...
use sigtrap           'handler', 
                      \&CRI::Test::_timed_out, 
                      'normal-signals';       # Catch kill signals and cleanup
use Proc::Daemon;
...
$SIG{ __DIE__ } = sub {
  ...
  elsif( $loc[0] =~ /^(?:main$|Moab::|CRI::|Torque::|Slurm::|Gold::)/ )
  {
    my $stack = Carp::longmess('STACKTRACE');

    unless( $stack =~ /haveCoreDump\(|::die\(/ || $message =~ /\bCOREDUMP\b/ )
    {
      $testStatus = 'ERROR';
      fail "ERROR: $message\n$stack";
      _finalizeReport();
...
sub _init #()
  ...
  if( !exists $ENV{ CRILogDir } || ( defined $ENV{ CRILogDir } && $ENV{ CRILogDir } !~ /^\S+$/ ) )
    {
  ...
  %initialHash = (
                     PTID        => $props->get_property(getppid().'.tid') || 0,
                     SCRIPT      => $curbin,
                     DESC        => "test$test_id",
                     TEST_RESULT => "RUNNING",
                     LOGFILE     => $logName,
...
sub _topLevelInit #()
  ...
  my @date      = localtime();
  my $timestamp = sprintf('%d-%02d-%02d_%02d.%02d.%02d', 
                          $date[5] + 1900, 
                          $date[4] + 1, 
                          $date[3], 
                          $date[2], 
  ...
  flock(FH, LOCK_EX) 
    or die "Bad stuff happened with the file lock on $full_props";
  $props->store(\*FH, "Last written to by $0 during test run with results in $ENV{'CRILogDir'}") 
...
sub _writeLog #($$)
  ...
  die "Lost logName value for some reason! .props = ".$props->get_property('Data.Props.Loc') unless defined $logName;
  ...
  my $retval = print _LOGFILE "${line_prefix}$logKey=$logValue\n"; # ascii values 19,20,15,16
  close(_LOGFILE) 
    or die("Couldn't close logfile : $!");
...
#------------------------------------------------------------------------------
#
# Performs a non-blocking wait for all pending zombie processes.
#
#------------------------------------------------------------------------------
sub reaper #()
  {
  ...
  while (($zombie = waitpid(-1, WNOHANG)) != -1) 
    ...
    } # END while (($zombie = waitpid(-1, WNOHANG)) != -1) 
...
sub _cleanExit #()
  ...
  foreach (@kidPids)
    ...
    if (kill(0, $_))
      ...
      # Sending kill 0 should return true if the process is running
      for my $kill ('TERM', 'INT', 'HUP', 'KILL')
        {
...
sub runCommand #($;%)
  ...
  $command = "ssh -o BatchMode=yes root\@$host '$command'" if defined $host;
  ...
  # Prevent random annoying communication errors from ruining perfectly good tests
  my $tries = 3;
  do 
  {  
      diag "Moab 'Communication Error' so Retrying Command..." if $tries < 3;
  ...
  }while($stderr =~ /^ERROR:\s+communication error.+cannot locate message body/m && $tries--);
  ...
  unless( exists $flags{no_checks} )
  {
    if ( $use_valg == 0 &&
      ...
      && ($stderr =~ /cannot load packet size|connection refused|core dumped|Aborted/
  ...
  if (exists $flags{ test_success } || exists $flags{ test_success_die })
    ...
    CRI::Test::die("Command '$command' failed")
    ...
    CRI::Test::die("Command '$command' did not fail as expected")
...
# 
# DESCRIPTION:
# Tests that two values are withing an expected tolerance. 
#
# PARAMETERS (IN ORDER):
...
sub cmp_tolr ($$$$;$)
  ...
  if ($tolr =~ /^([\-\+]?[\d\.]+)%$/)
    ...
    if( $rep == 0 && $diff != 0 )
    {
	$value = "INFINITE (divide by $rep)";
	$opr   = 'eq';
	$alert = "ALERT: Perhaps it would be more appropriate to use a non-percentange value in this case.\n"
    }
    elsif( $rep == 0 && $diff == 0 )
    {
	$value = $diff;
...
sub dynamic_cmp #(%)
{
  ...
  } # END elsif (ref $case{ expt } eq 'ARRAY' || ref $case{ expt } eq 'HASH')
  else
  {
  ...
  sub _log_is_deeply
  {
    ...
    elsif($ref2_type eq 'ARRAY')
    {
      ...
      foreach (0 .. $num)
      {
        ...
        {
          my $tmp = _log_is_deeply($ref1->[$_], $ref2->[$_], "$curr_place\[$_]");
          $cmp_string .= $tmp if defined $tmp;
        }
        else
        {
          $cmp_string .= &$format("$curr_place\[$_]", $ref1->[$_], $ref2->[$_]) if $ref2->[$_] ne $ref1->[$_];
        }
      }
    }
...
sub die #($)
  ...
  unless ($props->get_property('no.die'))
    ...
    } # END unless ($props->get_property('no.die'))
  else
    {
...
sub haveCoreDump #($)
{
  ...
  my %non_host_flags;
  $non_host_flags{$_} = $runcmd_flags{$_} foreach grep { $_ ne 'host' } keys %runcmd_flags;
...
#  my (%execs, %recover_subs);
  ...
  if (scalar @coredumps)
  {
    ...
    {
      # make sure coredump is closed
      my %wait;
      do
      {
        %wait = runCommand("lsof -w $coreDumpDir/$core", %runcmd_flags);
      }
      while( $wait{STDOUT} ne '' && sleep 1 );
      ...
      $cmd    = defined $runcmd_flags{host} ? "scp -o BatchMode=yes $runcmd_flags{host}:$cmd" : "mv $cmd";

      runCommand($cmd, no_coredump_check => 1 );
      runCommand("rm $coreDumpDir/$filename", %runcmd_flags) if defined $runcmd_flags{host};
      ...
      %cmd = runCommand("gdb $process_name $core_fullpath -x $gdb_cmd_file -q", %non_host_flags);
...
#      $recover_subs{$process_name} = sub{ runCommand($process_name, host => $runcmd_flags{host}); };
	...
	my @moab_files = ("$home/sbin/moab", "$home/log/moab.log", "$home/moab.cfg");

	foreach( @moab_files )
	{
	  if( defined $runcmd_flags{host} )
	  {
	    runCommand("scp -o BatchMode=yes $runcmd_flags{host}:$_ $outdir/. ", %runcmd_flags);
	  }
	  else
	  {
	    runCommand("cp $_ $outdir/. ", %runcmd_flags);
	  }
	...
	push @files, $filename;
      ...
      runCommand("tar --remove-files -C $outdir -cvzf $core_fullpath.tar.gz ".join(' ', @files), %non_host_flags) if scalar @files;
    }
...
sub checkValgrindXML
{
  ...
  my @filters = (
    qr"<stack>\s+(?:<frame>.+?<fn>malloc</fn>.+?</frame>\s+)*?<frame>.+?<obj>/lib/.*?/?(?:libc|ld)-.+?</obj>.+?</frame>"s,
    qr"<kind>UninitCondition</kind>",
    qr"<frame>\s+</frame>"s,
  );
  ...
  my @exe_checks = map { (/(\w+)\.do\.memcheck$/)[0] } grep { /\w+\.do\.memcheck$/ } $props->property_names();

  foreach my $prod (@exe_checks)
  {
    #diag "Checking Valgrind XML for $prod".$add_msg;
    my $outfile = $props->get_property("$prod.memcheck.log") || "/tmp/valgrind_$prod.out";
...
#    @errors = grep { sub{ eval{ parseXML($_); }; return $@ eq ''; }; } @errors;
    ...
    if( scalar @errors > 0 )
    {
      fail('Valgrind Found '.(scalar @errors).' Possible Problem(s) in '.ucfirst($prod)."${add_msg}! (see test log)");
...
END
  ...
  #  Sub-Note: Working on Fixing above Note in order to more fully debug bad tests -- Isaac Tobler 2010-04-30
  ...
  exit() 
	...
	$testStatus = "PASS" 
	if (@testResults and $testResults[0] == 1);
