...
sub job_info #($)
  {
     ...
     # Get the job info
     %qstat    = runCommand($cmd, test_success => 1);
...
sub node_info 
  ...
  my $cmd       = "pbsnodes -x";
  my %pbsnodes  = runCommand($cmd, test_success => 1);
...
=head1 SYNOPSIS
 ...
 my %result = runCommand('sleep 30', test_success => 1);
...
=item list2array (DEPRECATED: use CRI::Util::list2array)
