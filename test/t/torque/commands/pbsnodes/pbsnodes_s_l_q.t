...
foreach my $node (@nodes)
  ...
  $cmd         = "pbsnodes -s $server -l -q $node";
  %pbsnodes    = runCommand($cmd);
  $output      = $pbsnodes{ 'STDERR' };
  test_s_q_output( $output );
...
foreach my $property (@properties)
  ...
  $cmd         = "pbsnodes -s $server -l -q :$property";
  %pbsnodes    = runCommand($cmd);
  $output      = $pbsnodes{ 'STDERR' };
  test_s_q_output( $output );
...
foreach my $state (@states)
  ...
  $cmd         = "pbsnodes -s $server -l -q $state";
  %pbsnodes    = runCommand($cmd);
  $output      = $pbsnodes{ 'STDERR' };
  test_s_q_output( $output );
...
foreach my $state (@states)
  ...
  foreach my $node (@nodes)
    ...
    $cmd         = "pbsnodes -s $server -l -q $state $node";
    %pbsnodes    = runCommand($cmd);
    $output      = $pbsnodes{ 'STDERR' };
    test_s_q_output( $output );
...
foreach my $state (@states)
  ...
  foreach my $property (@properties)
    ...
    $cmd         = "pbsnodes -s $server -l -q $state $property";
    %pbsnodes    = runCommand($cmd);
    $output      = $pbsnodes{ 'STDERR' };
    test_s_q_output( $output );
