...
# Test Description
plan('no_plan');
setDesc('Momctl -p <PORT> -h <HOST> -q <QUERY>');
# Variables
my $momctl;
my $attr;
my $reg_exp;
...
while (($attr, $reg_exp) = each (%momctl_doc_tests)) 
  ...
  $momctl  = run_query($host, $port,  $attr);
...
foreach my $set_unset_test (@set_unset_tests)
  ...
  $query   = "$attr='$reg_exp'";
  $momctl  = run_query($host, $port,  $query);
  check_query_results($momctl, $host, $port,  $attr, $reg_exp);
...
$momctl = run_query($host, $port,  'cycle');
...
$momctl = run_query($host, $port,  'diag0');
test_level_0($momctl->{ 'STDOUT' });

$momctl = run_query($host, $port,  'diag1');
test_level_1($momctl->{ 'STDOUT' });

$momctl = run_query($host, $port,  'diag2');
test_level_2($momctl->{ 'STDOUT' });

$momctl = run_query($host, $port,  'diag3');
...
sub run_query #($$$)
 {
  ...
  my $cmd  = "momctl -p $port -h $host -q $attr";
