...
$momctl = run_query('diag0');
test_level_0($momctl->{ 'STDOUT' });

$momctl = run_query('diag1');
test_level_1($momctl->{ 'STDOUT' });

$momctl = run_query('diag2');
test_level_2($momctl->{ 'STDOUT' });

$momctl = run_query('diag3');
...
sub check_query_results #($$$$)
  ...
  # Perform the test
  ok($result, "Checking 'momctl -q $attr' output");
