...
# Test Description
plan('no_plan');
setDesc('Momctl -h <HOST> -r <CONFIGFILE>');
...
my $stdout;
my $check_cmd                = "momctl -h $host -q check_poll_time";

# Reconfigure mom to new configuration
%momctl = reconfig($host, $reconfig_file);
$stdout = $momctl{ 'STDOUT' };
ok($stdout =~ /reconfig successful on ${host}/i, "Checking output of 'momctl -h $host -r $reconfig_file'");
...
%momctl = reconfig($host, $config_file);
$stdout = $momctl{ 'STDOUT' };
ok($stdout =~ /reconfig successful on ${host}/i, "Checking output of 'momctl -h $host -r $config_file'");
...
sub reconfig #($)
  ...
  my ($host, $cfg_file) = @_;
  my %result;
  my $cmd = "momctl -h $host -r $cfg_file";
