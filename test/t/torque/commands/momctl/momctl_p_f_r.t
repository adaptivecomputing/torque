...
# Test Description
plan('no_plan');
setDesc('Momctl -p $port -f <HOSTLISTFILE> -r <CONFIGFILE>');
...
my $stdout;
my $check_cmd                = "momctl -p $port -f $host_list_file -q check_poll_time";

# Reconfigure mom to new configuration
%momctl = reconfig($port, $host_list_file, $reconfig_file);
$stdout = $momctl{ 'STDOUT' };
foreach my $host (@hosts)
  {
  ok($stdout =~ /reconfig successful on ${host}/i, "Checking output of 'momctl -p $port -f $host_list_file -r $reconfig_file' for $host");
  }
...
%momctl = reconfig($port, $host_list_file, $config_file);
$stdout = $momctl{ 'STDOUT' };
foreach my $host (@hosts)
  {
  ok($stdout =~ /reconfig successful on ${host}/i, "Checking output of 'momctl -p $port -f $host_list_file -r $config_file' for $host");
  }
...
sub reconfig #($)
  ...
  my ($port, $host_list_file, $cfg_file) = @_;
  my %result;
  my $cmd = "momctl -p $port -f $host_list_file -r $cfg_file";
