...
my $qthash = qstat_fx({flags => "-t"});
$qhash = qstat_fx();

foreach my $jaid (@jaids)
  {

  ok(defined $qthash->{ $jaid }, "Checking job:$jid1 for subjob:$jaid");
  cmp_ok($qthash->{ $jaid }{ 'job_array_request' }, 'eq', $id_exp, "Verifying 'job_array_request' for subjob:$jaid");
